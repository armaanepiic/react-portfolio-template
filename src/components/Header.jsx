...
export default function Header({ activeSection, scrollTo }) {
  ...
  const go = (item) => { scrollTo(item.toLowerCase()); setOpen(false); };

  return (
    <header className={`anim-nav fixed top-0 left-0 right-0 z-50 transition-all duration-500
      ${scrolled ? "glass shadow-xl shadow-black/50 py-3" : "py-5 bg-transparent"}`}>
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setOpen((o) => !o)}
            ...
            <span className={`block w-5 h-0.5 bg-current mt-1.5 transition-all duration-300 ${open ? "opacity-0" : ""}`} />
            <span className={`block w-5 h-0.5 bg-current mt-1.5 transition-all duration-300 ${open ? "-rotate-45 -translate-y-1.5" : ""}`} />
          </button>
        </div>
      </div>

      <div className={`md:hidden overflow-hidden transition-all duration-500 ${open ? "max-h-80 opacity-100" : "max-h-0 opacity-0"}`}>
        <ul className="mx-4 mt-3 mb-1 rounded-2xl px-5 py-4 flex flex-col gap-3 glass">
          {NAV.map((item) => (
            <li key={item}>
